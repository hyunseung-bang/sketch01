    ...
    function memberJoinNot({status, cardNo, joinDtFmt, reJoinDtFmt}) {
        ...
        let title, text, item;
        if( status === "AGE") {
            title = "under the age of 14";
            text = "You cannot register because you are under 14 years old.";
            item = "";
        } else if( status === "WDRL") {
            title = "a withdrawal member";
            text = "You left the company a while ago. You can sign up after 5 days of withdrawal.";
            item = `<div class="alert-infor"><div class="form-ipt row">
                <dl class="form-item"><dt class="entry">Re-subscribeable date and time</dt><dd class="comp">${reJoinDtFmt}</dd></dl>
            </div></div>`;
        } else if( status === "DUP") {
            title = "an existing member";
            text = "You cannot apply because you are an existing subscriber.";
            item = `<div class="alert-infor"><div class="form-ipt row">
                <dl class="form-item"><dt class="entry">Membership number</dt><dd class="comp">${cardNo}</dd></dl>
                <dl class="form-item"><dt class="entry">Date of subscription</dt><dd class="comp">${joinDtFmt}</dd></dl>
            </div></div>`;
        } else {
            alert("NO");
        }
        ...
        <div id="member-join-not" class="popup sm type1 motion up on">
            <div class="popup-inner">
                <div class="popup-header">
                    ...
                    <span class="btn-popup-close"><button type="button"><span class="hide">Close</span></button></span>
                </div>
                ...
                <div class="popup-btn">
                    <div class="popup-btn-inner">
                        <span class="btn btn-popup-close"><button type="button">Check</button></span>
                    </div>
                </div>
            </div>
        </div>
